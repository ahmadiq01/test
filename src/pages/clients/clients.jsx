...
const ContactPage = () => {
    ...
    useEffect(() => {
        ...
        // Add auto-scrolling class to the slider
        carouselRef.current.classList.add('auto-scrolling');
        
        // Pause animation when interacting with slider
        const pauseAnimation = () => setIsPaused(true);
        const resumeAnimation = () => setIsPaused(false);
        
        carouselRef.current.addEventListener('mouseenter', pauseAnimation);
        carouselRef.current.addEventListener('mouseleave', resumeAnimation);
        
        return () => {
            if (carouselRef.current) {
                carouselRef.current.removeEventListener('mouseenter', pauseAnimation);
                carouselRef.current.removeEventListener('mouseleave', resumeAnimation);
            }
        };
    }, []);
